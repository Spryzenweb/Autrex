...
export class ToolsDownloader {
  ...
  async downloadAndExtractTools(
    ...
    try {
      ...
      // On macOS, skip executable check since .exe files won't be executable
      // Just check if file exists
      const accessFlags =
        process.platform === 'darwin' ? fs.constants.F_OK : fs.constants.F_OK | fs.constants.X_OK

      const verifyInstall = await fs.promises
        .access(installedModTools, accessFlags)
        .then(() => true)
        .catch(() => false)
