...
export class GameDetector {
  ...
  private async detectMac(): Promise<string | null> {
    try {
      ...
      const commonPaths = [
        ...
        path.join(home, 'Applications/League of Legends.app/Contents/LoL/Game')
      ]
      ...
      // Try RiotClientInstalls.json in Application Support
      ...
      try {
        await fs.access(riotPath)
        const content = await fs.readFile(riotPath, 'utf-8')
        const data = JSON.parse(content)
        if (data.associated_client && typeof data.associated_client === 'object') {
          for (const [installPath] of Object.entries<string>(data.associated_client)) {
            if (installPath.toLowerCase().includes('league of legends')) {
              const gamePath = path.join(installPath, 'Contents', 'LoL', 'Game')
              if (await this.isValidGamePath(gamePath)) {
                return gamePath
              }
            }
      ...
      // Spotlight search as last resort
      try {
        const { stdout } = await execAsync(`mdfind 'kMDItemFSName == "League of Legends.app"'`)
        const lines = stdout
          .split('\n')
          .map((l) => l.trim())
          .filter(Boolean)
        for (const appPath of lines) {
          const gamePath = path.join(appPath, 'Contents', 'LoL', 'Game')
          if (await this.isValidGamePath(gamePath)) {
            return gamePath
          }
        }
      ...
      console.error('Failed to detect macOS game path:', error)
      return null
    }
  }
  ...
  private async isValidGamePath(gamePath: string): Promise<boolean> {
    try {
      // Check for Windows exe or macOS app bundle inside Game
      const exePath = path.join(gamePath, 'League of Legends.exe')
      const appPath1 = path.join(gamePath, 'LeagueofLegends.app')
      const appPath2 = path.join(gamePath, 'League of Legends.app')

      const [exeExists, app1Exists, app2Exists] = await Promise.all([
        fs
          .access(exePath)
          .then(() => true)
          .catch(() => false),
      ...
      return exeExists || app1Exists || app2Exists
    } catch (error) {
