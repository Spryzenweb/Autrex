...
export class GamePathService {
  ...
  private async isValidGamePath(gamePath: string): Promise<boolean> {
    try {
      ...
      const macAppPath1 = path.join(gamePath, 'LeagueofLegends.app')
      const macAppPath2 = path.join(gamePath, 'League of Legends.app')

      const [exeExists, macApp1Exists, macApp2Exists] = await Promise.all([
        fs.promises
          .access(exePath)
          .then(() => true)
          .catch(() => false),
      ...
      return exeExists || macApp1Exists || macApp2Exists
    } catch {
      return false
    }
  }
