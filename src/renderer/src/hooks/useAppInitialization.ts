...
export function useAppInitialization() {
  ...
  useEffect(() => {
    if (!window.api) {
      console.warn('window.api not available, skipping update listeners')
      return
    }
  ...
  useEffect(() => {
    if (!window.api) {
      console.warn('window.api not available, skipping settings listeners')
      return
    }
  ...
  useEffect(() => {
    if (!window.api) {
      ...
      setLeagueClientEnabled(true)
      setChampionDetectionEnabled(true)
      ...
      setSmartApplyEnabled(true)
      setAutoApplyEnabled(true)
      setAutoApplyTriggerTime(15)
