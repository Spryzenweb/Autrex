...
import { mockChampionData } from '../data/mockData'
...
export function useChampionData() {
  ...
  const loadChampionData = useCallback(
    async (preserveSelection = false) => {
      if (!isElectron()) {
        console.warn('Running in browser mode, using mock champion data')
        ...
        if (selectedChampionKey && selectedChampionKey !== 'all') {
          const champion = mockChampionData.champions.find((c) => c.key === selectedChampionKey)
          if (champion) {
        ...
        if (preserveSelection && selectedChampion) {
          const sameChampion = mockChampionData.champions.find(
            (c) => c.key === selectedChampion.key
          )
          if (sameChampion) {
  ...
  const fetchChampionData = useCallback(async () => {
    ...
    if (!isElectron()) {
      console.warn('Running in browser mode, fetch not available')
      setStatusMessage(t('status.browserMode') || 'Browser mode - fetch not available')
      return
    }
