...
export function UpdateDialog({ isOpen, onClose }: UpdateDialogProps) {
  ...
  useEffect(() => {
    ...
    if (!window.api) {
      console.warn('window.api not available, skipping update event listeners')
      return
    }
  ...
  const loadUpdateInfo = async () => {
    try {
      if (!window.api) {
        console.warn('window.api not available, cannot load update info')
        return
      }
  ...
  const handleDownload = async () => {
    if (!window.api) {
      console.warn('window.api not available, cannot download update')
      setError('Update functionality not available in browser mode')
      return
    }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>A new version of Autrex is available for download.</DialogDescription>
        </DialogHeader>
